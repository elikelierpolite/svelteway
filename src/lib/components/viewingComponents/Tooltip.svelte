<script>
	...
	import ToolTip1 from './mainComponents/ToolTip1.svelte';
			...
			swecode() {
				...
				this.code = this.mainComponent.helper.on
					? `<div class="${this.mainComponent.helper.classes.join(' ')}" data-tip="${
							...
							' '
					  )}" data-tip="hello"><button class="btn">Hover me</button></div></div>`
					: `<div class="${removeBorderClass.join(
							' '
					  )}" data-tip="hello"><button class="btn">Hover me</button></div>`;
			}
		};
		const sweid = uuidv4();

		let newCvElement = new cvElement('tooltip1', {
			swElementDataAttrId: sweid,
			classes: ['tooltip', 'my-6'],
			helper: {
				on: false,
				type: 'tooltip',
				classes: ['tooltip'],
